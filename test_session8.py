...
def test_access_rights_function():
    ...
    assert len(session8.decorator_factory('high')(func)()) == 4

    assert len(session8.decorator_factory('mid')(func)()) == 3

    assert len(session8.decorator_factory('low')(func)()) == 2

    assert len(session8.decorator_factory('no')(func)()) == 1

    assert session8.decorator_factory('random')(func)() == "Improper access keyword set"


def test_authenticate_function():
    @session8.authenticate("secret")
    def my_func():
        return "Amazing!"
